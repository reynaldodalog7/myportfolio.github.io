    ...
    class WPH_Rewrite_Process
        {
                                            ...
                                            if( is_multisite()   &&  $ms_settings['allow_every_site_to_change_options']  ==  'yes')
                                                {
                                                    ...
                                                    //retrieve the options for all active sites
                                                    foreach($sites_to_process   as  $site_to_process)
                                                        {
                                                            
                                                            switch_to_blog( $site_to_process->blog_id );
                                                            
                                                            $component_rewrite_data =   $this->_run_component_callback( $site_to_process->blog_id, $field_id, $_callback, $_callback_arguments, $_class_instance );
                                                                                                                                                                                    
                                                            $this->processed_data[]               =   $component_rewrite_data;
                                                            $this->map_sites_to_processed_data[]  =   $blog_id;
            ...
            private function _run_component_callback( $blog_id_settings, $field_id, $_callback, $_callback_arguments, $_class_instance )
                {
                    ...
                    if ( ! empty($_callback_arguments)  &&  is_array($_callback_arguments) &&   count($_callback_arguments) >   0 )
                        $module_mod_rewrite_rules   =   call_user_func_array( array($_class_instance, $_callback), array_merge( array( 'field_value'    =>  $saved_field_value), $_callback_arguments));
                        else
                    ...
                    foreach($this->processed_data    as  $response)
                        {
                            if(isset($response['rewrite']) &&  !empty($response['rewrite']))
                                {
                                    $this->rewrite_data[]   =   $response['rewrite'];
                                }
                        }
            ...
            function iis_process_rewrite_rules()
                {
                    ...
                    $status =   $this->iis_add_rewrite_rule( $this->rewrite_data, $web_config_file );
                    if ( $status    === FALSE )
                        {
                            $this->require_manual_setup_add_markers();
                            return;
            ...
            */
            ...
            /**
            * Add WriteCheckStrings to internal rewrite_data variable
            * 
            */
            private function iis_add_additionals()
                {
                            ...
                            if( in_array($blog_id, $this->sites_using_rewrite_rules) )
                                {
                                    ...
                                    array_unshift($this->rewrite_data, $rule);
                                }
                                else
                    ...
                    array_unshift(  $this->rewrite_data, "\n\n\n" . '<!--  BEGIN WP Hide & Security Enhancer -->');
                    array_push(     $this->rewrite_data, "\n" . '<!--  END WP Hide & Security Enhancer -->' . "\n\n\n");    
                }
            ...
            private function nginx_add_additionals()
                {
                    ...
                    if ( is_multisite() &&  defined('SUBDOMAIN_INSTALL')    &&  SUBDOMAIN_INSTALL   === FALSE )
                        {
                            ...
                            */
                            ...
                            $this->rewrite_data     =   array_merge( $wp_rewrite_data, $this->rewrite_data );
                        }
                    ...
                    $this->rewrite_data     =   array_merge( $this->rewrite_data, $custom_data );
            ...
            function generate_site_write_check(  )
                {
                    ...
                    $write_check_string                 =   date("Y-m-d_H:i:s") . '_' . mt_rand(100, 99999999);
            ...
            function apache_get_readable_rewrite_data()
                {
                    ...
                    if(count($this->rewrite_data)   >   0)
                        {
                            foreach($this->rewrite_data as  $_htaccess_data_line)   
                                {
                                    $rules .=   "\n"    .   $_htaccess_data_line;
                                }                            
                        }
            ...
            function iis_get_readable_rewrite_data()
                {
                    ...
                    if(count($this->rewrite_data)   >   0)
                        {
                            foreach($this->rewrite_data as  $rewrite_line)   
                                {
                                    $rules .=   "\n"    .   $rewrite_line;
                                }                            
                        }
            ...
            function nginx_get_readable_rewrite_data()
                {
                    ...
                    $rewrite_rules  =   call_user_func_array('array_merge', $this->rewrite_data);
                                                                            ...
                                                                            foreach($unique_sites_rules as  $key    =>  $unique_sites_rule)
                                                                                {
                                                                                    if( is_multisite() &&  $ms_settings['allow_every_site_to_change_options']  ==  'yes')
                                                                                        {
                                                                                            $rules_sites_slug_map   =   array();
                                                                                            foreach ( $similar_rules_blocks as  $site_rules )
                                                                                                {
                                                                                                    foreach($site_rules['data'] as  $rule_data)
                                                                                                        {
                                                                                                            if ( $unique_sites_rule ==  trim($rule_data) )
                                                                                                                {
                                                                                                                    $blog_details   =   get_blog_details($site_rules['blog_id']);
                                                                                                                    $rules_sites_slug_map[] =   trim($blog_details->path, '/');   
                                                                                                                }
                                                                                                        }
                                                                                                }
                                                                                            
                                                                                            $_replacement_tags['__WPH_SITES_SLUG__']    =   $this->_get_slug_map( $rules_sites_slug_map, array( 'append_slash'   =>  TRUE ) );   
                                                                                        }
                                                                                        else
                                                                        ...
                                                                        if( is_multisite() &&  $ms_settings['allow_every_site_to_change_options']  ==  'yes')
                                                                            {
                                                                                                ...
                                                                                                if ( $unique_sites_rule ==  trim($rule_data) )
                                                                                                    {
                                                                                                        $blog_details   =   get_blog_details($site_rules['blog_id']);
                                                                                                        $rules_sites_slug_map[] =   trim($blog_details->path, '/');
                                                                                                        $rules_sites_host_map[] =   trim($blog_details->domain, '/');
                                                                                                    }
                                                                                            }
                                                                                    }
                                                                                ...
                                                                                $_replacement_tags['__WPH_SITES_SLUG__']    =   $this->_get_slug_map( $rules_sites_slug_map, array( 'append_slash'   =>  TRUE ) );   
                                                                                $_replacement_tags['__WPH_SITES_HOST__']    =   $this->_get_slug_map( $rules_sites_host_map );
                                                                            }
                                                                            else
                                                                            {
                                                                                $rule_data  =   $unique_sites_rules_raw[$key];
            ...
            private function _get_slug_map( $rules_sites_slug_map, $attr   =   array() )
                {
                    ...
                    if ( count ($rules_sites_slug_map ) >   1 )
                        {
                            $map_text   =   '(';
                            
                            if ( $attr['append_slash'] )
                                $map_text   .=  '\/';
                            
                            if ( $attr['append_slash'] )
                                $map_text   .=   implode('|\/', $rules_sites_slug_map );
                                else
                                $map_text   .=   implode("|", $rules_sites_slug_map );
                                
                            $map_text   .=  ')' . $regex_optional; 
                        }
                        else if ( count ($rules_sites_slug_map ) >   0 )
                            {
                                if ( $attr['append_slash'] )
                                    {
                                        if ( ! empty ( $regex_optional ) )
                                            $map_text   .=  '(\/';
                                            else
                                            $map_text   .=  '\/';
