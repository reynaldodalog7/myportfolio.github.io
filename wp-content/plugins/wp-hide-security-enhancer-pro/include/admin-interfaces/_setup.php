<?php   
        ...
        <div id="wph" class="wrap">
            <h1><span class="dashicons dashicons-admin-tools"></span> WP Hide & Security Enhancer - Setup</h1>
            <br />
            <div class="start-container">
                <div class="text">
                                    ...
                                    if (    $wph->functions->server_is_wpengine() )
                                        {
                                            ...
                                            <p>- <?php _e('Ask the support represendative to add the following Nginx rewrites to your account at "before" (not "before-in-location") spot! This can be for Staging or Production, depends on which the current site is deployed.', 'wp-hide-security-enhancer') ?></p>
                                    ...
                                    else if (    $wph->functions->server_is_kinsta() )
                                        {
                                            ...
                                            <p>- <?php _e('Ask the support represendative to add the following Nginx rewrites to your account! This can be for Staging or Production, depends on which the current site is deployed.', 'wp-hide-security-enhancer') ?></p>
                                            <p>- <?php _e('Explicitly specify the rewrite code must be used as is, they are not redirects but re-mapping rules.', 'wp-hide-security-enhancer') ?></p>
                                            <?php
                                        }
                                        else
                    ...
                    <p><?php _e('There is no change in rewrite data, no additional action is necessarily.', 'wp-hide-security-enhancer') ?></p>
            ...
            ?>">
                ...
                <?php
                    ...
                    if (    isset($readable_processed_rewrite['default_variables']) ||  isset($readable_processed_rewrite['location']) )
                        {    
            ...
            <div id="ruc-actions">
                <!-- WPH Preserve - Start -->
                <input name="submit" id="ruc-submit" class="button button-primary" value="Confirm" type="submit" onClick="rewrite_save_confirm('<?php $home_url   =   preg_replace('/:[0-9]+/', '', str_replace(array ("https:" , "http:"), "", home_url())); echo trailingslashit($home_url); ?>index.php', '<?php echo trailingslashit ( $home_url ) . $wp_login_slug; ?>', '<?php echo wp_create_nonce( 'ruc-nonce' ); ?>')" />
                <!-- WPH Preserve - Stop -->
...
<script type="text/javascript">
    ...
    function rewrite_save_confirm ( home_url, login_url, _nonce ) 
        {
                        ...
                        if ( $environment_status    ==  'Incorect' )
                            {
                                ...
                                ?>    
                            <p><br /><?php _e('The environment file contain correct data, no additional action is necessarily.', 'wp-hide-security-enhancer') ?></p>
                            <?php } ?>
                </div>
            </div>
            <br />
                ...
                <div class="text">
                    ...
                    <?php
                        ...
                        if ( $wp_config_status    ==  'Incorect' )
                            {
                                ?>
                                <p><?php _e('Add the following code to ', 'wp-hide-security-enhancer'); ?> <code><?php echo $wph->functions->get_wp_config_path() ?></code> <?php _e('<strong>Replacing</strong> any existing lines betwen <strong># START WP Hide & Security Enhancer</strong> and <strong># END WP Hide & Security Enhancer</strong>', 'wp-hide-security-enhancer') ?>:</p>
                                <?php   
                            }
                            else { 
                                $disabled   =   'disabled="disabled"';
                                ?>    
                            <p><br /><?php _e('The wp-config.php file contain correct data, no additional action is necessarily.', 'wp-hide-security-enhancer') ?></p>
                            <?php } ?>
                </div>
            </div>
            <br />
