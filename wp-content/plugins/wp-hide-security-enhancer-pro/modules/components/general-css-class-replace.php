<?php
    ...
    class WPH_module_general_css_class_replace extends WPH_module_component
        {
            ...
            function get_module_component_settings()
                {
                    ...
                    $this->component_settings[]                  =   array(
                                                                    'id'            =>  'css_class_replace',
                                                                    'label'         =>  __('CSS Class Replace',    'wp-hide-security-enhancer'),
            ...
            function _module_option_html( $module_setting )
                {
                    ...
                    <div id="replacer_read_root" style="display: none">
                        <p><input name="<?php echo $module_setting['id'] ?>[replaced][]" class="<?php echo $class ?>" value="" placeholder="Class to be Replaced" type="text"> <span alt="f345" class="dashicons dashicons-arrow-right-alt2">&nbsp;</span> <input name="<?php echo $module_setting['id'] ?>[replace][]" class="<?php echo $class ?>" value="" placeholder="Class to Replace" type="text"> <a href="javascript: void(0);" onClick="WPH.replace_text_remove_row( jQuery(this).closest('p'))"><span alt="f335" class="close dashicons dashicons-no-alt">&nbsp;</span></a> </p>
                    </div>
                    <?php
                    
                    $values =   $this->wph->functions->get_site_module_saved_value('css_class_replace',  $this->wph->functions->get_blog_id_setting_to_use(), 'display');
                    ...
                    if ( count ( $values )  >   0 )
                        {
                            foreach ( $values   as  $block)
                                {
                                    ?><p>
                                        <input name="<?php echo $module_setting['id'] ?>[replaced][]" class="<?php echo $class ?>" value="<?php echo htmlspecialchars(stripslashes($block[0])) ?>" placeholder="Class to be Replaced" type="text"> <span alt="f345" class="dashicons dashicons-arrow-right-alt2">&nbsp;</span> 
                                        <input name="<?php echo $module_setting['id'] ?>[replace][]" class="<?php echo $class ?>" value="<?php echo htmlspecialchars(stripslashes($block[1])) ?>" placeholder="Class to Replace" type="text"> 
